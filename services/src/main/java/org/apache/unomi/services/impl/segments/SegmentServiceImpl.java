...
public class SegmentServiceImpl extends AbstractServiceImpl implements SegmentService, SynchronousBundleListener {
    ...
    public Segment getSegmentDefinition(String segmentId) {
        ...
        if (definition != null && definition.getMetadata() != null && definition.getMetadata().isEnabled()) {
    ...
    public void setSegmentDefinition(Segment segment) {
        if (segment.getMetadata() == null) {
            throw new IllegalArgumentException("Segment metadata cannot be null for segment " + segment.getItemId());
        }
        if (segment.getMetadata().isEnabled()) {
            boolean conditionTypesResolved = ParserHelper.resolveConditionType(definitionsService, segment.getCondition(), "segment " + segment.getItemId());
            if (!conditionTypesResolved) {
                throw new BadSegmentConditionException(
                        "Segment '" + segment.getItemId() + "' references one or more unresolvable condition types");
            }
            if (!persistenceService.isValidCondition(segment.getCondition(), new Profile(VALIDATION_PROFILE_ID))) {
                throw new BadSegmentConditionException(
                        "Segment '" + segment.getItemId() + "' has an invalid condition");
            }
    ...
    public void setScoringDefinition(Scoring scoring) {
        if (scoring.getMetadata() == null) {
            throw new IllegalArgumentException("Scoring metadata cannot be null for scoring " + scoring.getItemId());
        }
        if (scoring.getMetadata().isEnabled()) {
            for (ScoringElement element : scoring.getElements()) {
